                        ...
                        <table id="datatables" class="table table-striped table-no-bordered table-hover nowrap" cellspacing="0" width="100%" style="width:100%">
                            ...
                            @foreach ($pagos as $pago)
                                <tr>
                                    ...
                                    <td data-order="{{ $pago->fecha_pago->timestamp }}">{{ $pago->fecha_pago->format('d/m/Y H:i') }}</td>
                                    <td>@if (isset($pago->cajero)) {{ $pago->cajero->nombres }} @endif</td>
                                    <td data-order="{{ $pago->monto }}">$ {{ number_format($pago->monto, 2, ',', '.')  }}</td>
                                    <td>{{ $pago->tipo_pago }}</td>
                                    <td>{{ $pago->descripcion }}</td>
                                    @if (Auth::user()->isSuperAdmin())
                                        <td>@if (isset($pago->service->subscriber)){{ $pago->service->subscriber->agency->isp->nombre }}@endif</td>
                                    @endif
                                    @if (Auth::user()->isSuperAdmin() || Auth::user()->isAdmin())
                                        <td>@if (isset($pago->service->subscriber)){{ $pago->service->subscriber->agency->nombre }}@endif</td>
                                    @endif
                                    @if ($pago->trashed())
                                    ...
                                    <td class="text-center">
                                        <span class="table-remove">
                                            <a class="btn btn-primary btn-fab btn-fab-mini btn-round" data-toggle="tooltip" title="Imprimir Recibo"
                                               href="{{ action('PaymentController@printTicket', $pago->id) }}" target="_blank"><i class="material-icons">print</i></a>
                                        </span>
                                        {{--<span class="table-remove">
                                            <a class="btn btn-primary btn-fab btn-fab-mini btn-round" data-toggle="tooltip" title="Imprimir Recibo"
